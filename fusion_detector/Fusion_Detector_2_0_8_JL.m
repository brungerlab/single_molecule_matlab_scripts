...
for i=1:length(dataStruct)
    ...
    for j=1:sizeChan1x
    	if j-peakDetBL1-preBaseline1>0 && j+peakDetpk1+peakDetPostBL1+peakDetDelay1<sizeChan1x
            ...
            peakstd(j,i)=std(chan1Baseline(j:j+peakDetpk1-1,i));
            ...
            StartBaseline(i)=mean(chan1Baseline(1:30,i));
            StartStd(i)=std(chan1Baseline(1:30,i));
            FinishBaseline(i)=mean(chan1Baseline(sizeChan1x-30:sizeChan1x,i));
            FinishSTD(i)=std(chan1Baseline(sizeChan1x-20:sizeChan1x,i));
               ...
               % if peak(j,i)>baselineVal(j,i)+noiseThresh(j,i) && delayAmplitude(j,i)>baselineVal(j,i)+delayNoiseThresh(j,i) && stdPeak(j,i)<hitnoise*(stdPeak(j,i))                           && PreBaselineVal(j,i)>baselineVal(j,i)-noiseThresh(j,i) && all(delayAmplitude(j-3:j,i)>baselineVal(j-3:j,i)+delayNoiseThresh(j-3:j,i)) && EndCheck(1,i)>BeginCheck(1,i)+BeginStdev(1,i) && (baselineVal(j,i))>BeginCheck(1,i)-(BeginStdev(1,i)*noiseMul2) && (EndCheck(1,i))>(PreBaselineVal(j,i)+noiseThresh(j,i)) && mean(peak(j-14:j-6,i))>BeginCheck(1,i)-BeginStdev(1,i) && EndCheck(1,i)>baselineVal(j,i)+noiseThresh(j,i)
                if FinishBaseline(i)+-StartBaseline(i)<0 || (FinishBaseline(i)-FinishSTD(i))<(StartBaseline(i)+StartStd(i)) || (FinishBaseline(i)-StartBaseline(i))<StartStd(i)*noiseMul1 || peak(j,i)<(StartBaseline(i)-(StartStd(i)*5)) || FinishBaseline(i)+(FinishSTD(i))<peak(j,i)-peakstd(j,i);
                    break             
                elseif peak(j,i)>baselineVal(j,i)+noiseThresh(j,i) && delayAmplitude(j,i)>baselineVal(j,i)+delayNoiseThresh(j,i) && stdPeak(j,i)<hitnoise*(std(chan1Baseline(j-peakDetBL1:j-1,i))) && PreBaselineVal(j,i)>baselineVal(j,i)-noiseThresh(j,i) && EndCheck(1,i)>BeginCheck(1,i)+BeginStdev(1,i) && (EndCheck(1,i))>(PreBaselineVal(j,i)+noiseThresh(j,i)) && EndCheck(1,i)>baselineVal(j,i)+noiseThresh(j,i) && peak(j,i)>StartBaseline(i)+StartStd(i) && abs(delayAmplitude(j,i)-peak(j,i))<peakstd(j,i); 
                        indexData(z,i)=j;
                        z=z+1;
                end
    	end
...
%% Run ChangeptsFusionDetector1.m on channel 1 and remove hits that did not pass
run('ChangeptsFusionDetector1.m');
%% Remove 
putativeHitsFinal=struct('name',[],'time', [], 'amplitude', [], 'datasetIndex', [], 'timeIndex', [], 'Validated', []);
for i=1:length(putativeHits)
    if putativeHits(i).Validated==1;
        if isempty(putativeHitsFinal(1).name);
            putativeHitsFinal=putativeHits(i);
        else
       putativeHitsFinal=[putativeHitsFinal,putativeHits(i)];
        end
    end
end
...
if ploton>0
    ...
    while i<=length(putativeHitsFinal)
    	if i>0
        	% Plots the current trace
            figPlot=figure(i+3);
            set(figPlot,'name','Hit?','numbertitle','off')
        	plot(rawTraces(:,1), (chan1Baseline(:,putativeHitsFinal(i).datasetIndex)), 'k', 'LineWidth', 1.5)
        	hold on
            plot((rawTraces(putativeHitsFinal(i).timeIndex,1)),(chan1Baseline(putativeHitsFinal(i).timeIndex,putativeHitsFinal(i).datasetIndex)), 'r.', 'markers',38)
        	ylabel('Amplitude');
        	xlabel('Time(s)');
        	title([putativeHitsFinal(i).name, ' ', 'at ', num2str(putativeHitsFinal(i).time), ' sec'])
        	...
        	% "exampleTraces."
            elseif key==115 
                exampleTraces(ETLoop).name=putativeHitsFinal(i).name;
                exampleTraces(ETLoop).time=putativeHitsFinal(i).time;
                exampleTraces(ETLoop).amplitude=putativeHitsFinal(i).amplitude;
                exampleTraces(ETLoop).datasetIndex=putativeHitsFinal(i).datasetIndex;
                exampleTraces(ETLoop).timeIndex=putativeHitsFinal(i).timeIndex;
            	ETLoop=ETLoop+1;
                i=i+1;
        ...
        if i==length(putativeHitsFinal)+1
            textBox2();
            if cancel3<2;
                i=i-1;
                falsePosNum(falsePosNum==i)=[];              
...
% "putativeHits."
confirmedHits=putativeHitsFinal;
if ploton>0
    if falsePosNum>0
...
for i=1:length(dataStruct)
    ...
    for j=1:sizeChan1x
    	if j-peakDetBL2-preBaseline2>0 && j+peakDetpk2+peakDetPostBL2+peakDetDelay2<sizeChan1x
            ...
            peakstd(j,i)=std(chan2Baseline(j-peakDetpk2*(3/8):j+peakDetpk2*(1/2),i));
            ...
            StartBaseline(i)=mean(chan2Baseline(1:30,i));
            StartStd(i)=std(chan2Baseline(1:30,i));
            FinishBaseline(i)=mean(chan2Baseline(sizeChan1x-20:sizeChan1x,i));
            FinishSTD(i)=std(chan2Baseline(sizeChan1x-20:sizeChan1x,i));
            	% This is the actual peak detection. It stores the index
            	% infomation for the putative hits into a variable called
            	% indexData.
               % if peak(j,i)>baselineVal(j,i)+noiseThresh(j,i) && delayAmplitude(j,i)>baselineVal(j,i)+delayNoiseThresh(j,i) && stdPeak(j,i)<hitnoise*(stdPeak(j,i))                           && PreBaselineVal(j,i)>baselineVal(j,i)-noiseThresh(j,i) && all(delayAmplitude(j-3:j,i)>baselineVal(j-3:j,i)+delayNoiseThresh(j-3:j,i)) && EndCheck(1,i)>BeginCheck(1,i)+BeginStdev(1,i) && (baselineVal(j,i))>BeginCheck(1,i)-(BeginStdev(1,i)*noiseMul2) && (EndCheck(1,i))>(PreBaselineVal(j,i)+noiseThresh(j,i)) && mean(peak(j-14:j-6,i))>BeginCheck(1,i)-BeginStdev(1,i) && EndCheck(1,i)>baselineVal(j,i)+noiseThresh(j,i)
...
for i=1:length(row)
 ...
 %% Run ChangeptsFusionDetector1.m on channel 1 and remove hits that did not pass
run('ChangeptsFusionDetector2.m');
%% Remove 
putativeHitsFinal2=struct('name',[],'time', [], 'amplitude', [], 'datasetIndex', [], 'timeIndex', [], 'Validated', []);
for i=1:length(putativeHits2)
    if putativeHits2(i).Validated==1;
        if isempty(putativeHitsFinal2(1).name);
            putativeHitsFinal2=putativeHits2(i);
        else
       putativeHitsFinal2=[putativeHitsFinal2, putativeHits2(i)];
        end
    end
end
%% Plot data and select true hits from putative hit list
...
% variable "i" indicates which traces is being plotted.
while i<=length(putativeHitsFinal2)
    if i>0
        % Plots the current trace
        figPlot=figure(i+3);
        set(figPlot,'name','Hit?','numbertitle','off')
        plot(rawTraces(:,1), (chan2Baseline(:,putativeHitsFinal2(i).datasetIndex)), 'k', 'LineWidth', 1.5)
        hold on
        plot((rawTraces(putativeHitsFinal2(i).timeIndex,1)),(chan2Baseline(putativeHitsFinal2(i).timeIndex,putativeHitsFinal2(i).datasetIndex)), 'r.', 'markers',38)
        ylabel('Amplitude');
        xlabel('Time(s)');
        title([putativeHitsFinal2(i).name, ' ', 'at ', num2str(putativeHitsFinal2(i).time), ' sec'])
        ...
        elseif key==8
            ...
            % "exampleTraces2."
        elseif key==115
            exampleTraces2(ETLoop).name=putativeHitsFinal2(i).name;
            exampleTraces2(ETLoop).time=putativeHitsFinal2(i).time;
            exampleTraces2(ETLoop).amplitude=putativeHitsFinal2(i).amplitude;
            exampleTraces2(ETLoop).datasetIndex=putativeHitsFinal2(i).datasetIndex;
            exampleTraces2(ETLoop).timeIndex=putativeHitsFinal2(i).timeIndex;
            ETLoop=ETLoop+1;
            i=i+1;
    ...
    % asking if the user is done looking through the traces. If not,
    % the user will be returned to the last trace.
    if i==length(putativeHitsFinal2)+1
        textBox2();
        if cancel3<2;
            i=i-1;
            falsePosNum(falsePosNum==i)=[];
...
% positives identified by the user. These traces are removed from 
% "putativeHits2."
confirmedHits2=putativeHitsFinal2;
if ploton>0
if falsePosNum>0
    for i=length(falsePosNum):-1:1
